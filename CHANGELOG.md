...
- Reintegrate appended doc text into coherent sections: [lifecycle](docs/concepts/lifecycle.md) Day 2 splits scale-down, control-plane rollouts, config applies and versions into short subsections; the [ArgoCD guide](docs/configuration/argocd.md) moves the YAML-serializer guarantee into the intro and drops the per-scalar enumeration; [machines](docs/concepts/machines.md) states the Path B VIP exclusion once and describes the Proxmox duplicate-VM-name collision as a managed-name abort with unmanaged collisions ignored; [plugins](docs/concepts/plugins.md) states the results-forwarding rule once and points the other section at it.
- Document the new fail-closed aborts in the [troubleshooting guide](docs/troubleshooting.md) where operators look for them — the etcd-member control-plane removal refusal, the unknown-Kubernetes-version machine-config refusal, duplicate Proxmox VM names, the Rancher unresolved-member and downstream `id_mismatch_reason` refusals, and the SDN teardown refusal on shared-controller pending state — and note in [Usage](docs/usage.md) that scale-down also removes owned machines that never joined or whose VM delete failed.
- Correct the configuration-overview validation summary: a miscapped or unsupported key — nested fixed-schema blocks such as `proxmox.network` included — is refused, and each plugin's `validate` hook runs in converge's validate phase for every installed plugin before any mutation.
- Correct the [ArgoCD configuration guide](docs/configuration/argocd.md) to describe both Rancher cluster-id resolution paths the same way.
- Align the docs with the incomplete-`check` exit/report contract: [Usage](docs/usage.md) no longer says unavailable upstream/node data can still exit `0`, the stale "check succeeds while version data is unavailable" troubleshooting section is dropped, and the quickstart's obsolete pre-bootstrap plugin-planning workaround is removed.
- Correct the backup guide's claim that a backup predating bootstrap is not the current identity, and scope the operational-guide client commands to the generated files and selected directory.
- Correct the blocked-drain recovery instruction in the [troubleshooting guide](docs/troubleshooting.md) to match maintenance: loosen a tight PDB by lowering `minAvailable` or raising `maxUnavailable` (or adding replicas above the PDB floor).
- Refuse Rancher converge when the downstream `cattle-cluster-agent` id matches no Rancher cluster bearing the configured name, instead of creating a fresh import cluster that can never match the agent.
- Run the Proxmox SDN teardown guards — pending administrators' changes and the shared controller's pending `deleted` or `changed` — before destroy deletes any VM or the resource pool, so `plan`/dry-run reports a refused teardown and a refused destroy leaves the SDN network and identity behind.
- Record ArgoCD `check`/`status` Rancher cluster ids in `ctx.results` and read the downstream `cattle-cluster-agent` id in a standalone run, so check renders the same id a converge published instead of an empty one that reported drift.
- Render every ArgoCD user-controlled scalar through a YAML serializer, so a value with special characters round-trips exactly (instead of a broken or mistyped manifest) and single-line scalars stay on one physical line.
- Require the Rancher `BackingNamespaceCreated` condition's `status` to equal the literal `"True"` before treating a cluster's backing namespace as ready.
- Make Rancher `check`/`status` enforce the same downstream identity match as converge and destroy, reporting `id_match: false` and an `id_mismatch_reason`, instead of treating any nonempty downstream id as the installed agent.
- Refuse a miscapped or unsupported key inside every fixed-schema section of `cluster.yaml` and `secrets.yaml` — including `proxmox.network` and its subsections — instead of silently ignoring it and falling back to a default; freeform maps are preserved.
- Normalize the `v` prefix on `kubernetes.version` as well as the Talos version, so an unprefixed pin is canonicalized and no longer schedules a perpetual upgrade or renders a component image tag that lacks the leading `v`.
- Fail Rancher membership reconciliation when a configured member cannot be resolved, instead of removing an existing binding as stale.
- Reject duplicate Proxmox VM names that involve a cluster-managed machine before converge or destroy, while same-named unowned VMs are left alone.
- Exclude floating kube-api VIPs from every node-address fallback, so a bridge-mode cluster without Tailscale never targets whichever control plane owns the VIP.
- Refuse to generate or apply target-version machine configs when the running cluster's Kubernetes version cannot be read; a dry run with no `kubeconfig` on disk still keeps the target so a recovered management machine's `plan` completes.
- Reconcile scale-down removals from the owned provider inventory as well as the live Kubernetes node set, and let a stranded control plane with a dead or wiped node be removed once the surviving control planes confirm it left etcd.
- Report a configured machine that shows up in neither Talos discovery nor the Kubernetes node list as an incomplete `check`.
- Reject Rancher memberships where two netids across different tiers resolve to the same principal.
- Add regression tests for the risky converge paths that had none: upgrade aborts, even-count control-plane scale-down, endpoint-move completion, machine-config phase ordering, k8s minor stepping, SDN drift updates, the Proxmox task timeout and cidata cleanup, and the Rancher HTTP verbs.
- Keep the reorganized docs' URLs honest: a test asserts every absolute GitHub Pages link in `README.md` resolves to a published page and heading, and the established `/concepts/*` URLs stay stable.
- Add a repeatable live workflow test ([scripts/live-workflow-test.sh](scripts/live-workflow-test.sh) and [docs/testing.md](docs/testing.md)) that drives the real CLI against a disposable cluster, and clarify the guide's provider and tailscale requirements.
- Turn the documentation audit's ad-hoc checks into maintained tests: the documented `cluster.yaml` example loads with matching secrets, every internal doc link and anchor resolves, and `docs/commands.md` covers every CLI subcommand and its aliases.
