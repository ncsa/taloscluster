...
- Add `metal` commands that inspect, boot, wait, apply, eject and join bare-metal machines, previewable with `--dry-run`, refusing an already-joined machine and one no probe could identify unless `--force`, and skipping the BMC when redfish is disabled. `metal apply` generates the machine config against the cluster endpoint the provider resolved — refusing until converge has run — writes it to `.metal/` (which `init` git-ignores) at mode 0600 and warns when the directory is not ignored.
...
- Encrypt every installed machine's STATE and EPHEMERAL partitions with LUKS2, boot Proxmox VMs with Secure Boot, vendor the bootstrap manifests, and require https tailscale login servers.
