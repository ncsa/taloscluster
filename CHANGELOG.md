...
- Join a bare-metal machine missing from the cluster during converge only when its group sets `auto_join: true`, never reinstalling one that already answers apid with the cluster's identity or that no probe could identify; `plan` lists the machines it would join and converge exits 1 when one fails to join.
