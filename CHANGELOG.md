...
- Reintegrate appended doc text into coherent sections: [lifecycle](docs/concepts/lifecycle.md) Day 2 splits scale-down (worker cleanup separate from control-plane etcd quorum), control-plane rollouts, config applies and versions into short subsections; the [ArgoCD guide](docs/configuration/argocd.md) moves the YAML-serializer guarantee into the intro and drops the per-scalar enumeration; [machines](docs/concepts/machines.md) states the Path B VIP exclusion once and describes the Proxmox duplicate-VM-name collision as a managed-name abort with unmanaged collisions ignored; [plugins](docs/concepts/plugins.md) states the results-forwarding rule once and points the other section at it.
- Correct the docs on the one exception to the unknown-version machine-config refusal: the escape is a dry run with no non-empty `kubeconfig` on disk (recovered or not — a `plan` after deleting `kubeconfig` next to a reachable cluster also keeps the target and shows target-version diffs), and `_config_kubernetes_version` no longer claims an unreachable kube-api falls back to `cluster.yaml` (it aborts). See [version selection](taloscluster/converge.py).
- Document the new fail-closed aborts in the [troubleshooting guide](docs/troubleshooting.md) where operators look for them — the etcd-member control-plane removal refusal ("still an etcd member ... could cost quorum"), the unknown-Kubernetes-version machine-config refusal ("could not determine the running cluster's kubernetes version"), duplicate Proxmox VM names among cluster-managed machines, the Rancher unresolved-member refusal and the downstream `id_mismatch_reason`, and the SDN teardown refusal on shared-controller pending state — and note in [Usage](docs/usage.md) Scale that scale-down also removes owned machines that never joined or whose VM delete failed.
- Correct the self-contradicting configuration-overview validation summary: a miscapped or unsupported key is refused (and nested fixed-schema blocks such as `proxmox.network` are refused too, not just top-level ones), and each plugin's `validate` hook runs in converge's validate phase for every installed plugin before any mutation. See [plugin validation](taloscluster/plugins.py) and [config loading](taloscluster/config.py).
- Correct the [ArgoCD configuration guide](docs/configuration/argocd.md) intro to describe both Rancher cluster-id resolution paths the same way: a shared converge takes the id from the preceding rancher hook, and a standalone run reads it off the downstream `cattle-cluster-agent`, dropping the stale "ArgoCD alone has no preceding Rancher result" claim. See [ArgoCD rancher resolution](plugins/argocd/taloscluster_argocd/reconcile.py).
