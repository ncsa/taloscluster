...
The Proxmox server origin. The `/api2/json` path is added internally; URLs that already include it still work. The cluster must run Proxmox 9 or newer; the release is read from the first node whenever the cluster's inventory loads, so `destroy` is refused below Proxmox 9 the same way converge is: Proxmox 9 makes a VM NIC inherit its bridge's MTU from an unset MTU, while 8 and earlier default it to 1500 and need an `mtu=1` sentinel to inherit — the opposite convention, and one 9 reads as a literal MTU of 1, which costs the node its network. Tear down a Proxmox 8 cluster with taloscluster 0.7.x, which still accepts it. See the [Proxmox 9.0 known issues](https://pve.proxmox.com/wiki/Roadmap#9.0-known-issues).
