...
The rancher plugin imports the cluster into a Rancher server, installs the cluster agent, and reconciles the members listed here. It is skipped, and shown as `not configured` by `taloscluster plugin list`, unless a `rancher` section in `cluster.yaml` or an included file opts the cluster into Rancher management and the merged configuration carries both `url` and `token`. Where each value lives is your choice — [secrets.yaml and included files merge into `cluster.yaml`](../configuration.md#secretsyaml), so the split below is only the scaffolded default and the credentials may equally sit in `cluster.yaml` or an included file. See [Plugins](../concepts/plugins.md#how-plugins-run) for what converge, destroy and check do. When the downstream agent is orphaned (matches no existing Rancher cluster), converge refuses to create a fresh registration that can never match it; when a cluster bearing the configured name has a different id, it refuses to attach to that cluster; see [Rancher cluster id no longer matches the downstream agent](../troubleshooting.md#rancher-cluster-id-no-longer-matches-the-downstream-agent).
