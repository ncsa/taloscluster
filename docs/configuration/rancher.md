...
The rancher plugin imports the cluster into a Rancher server, installs the cluster agent, and reconciles the members listed here. It is skipped, and shown as `not configured` by `taloscluster plugin list`, unless `cluster.yaml` has a `rancher` section and `secrets.yaml` has both `url` and `token`. See [Plugins](../concepts/plugins.md#how-plugins-run) for what converge, destroy and check do. When the downstream agent is orphaned (matches no existing Rancher cluster), converge refuses to create a fresh registration that can never match it; when a cluster bearing the configured name has a different id, it refuses to attach to that cluster; see [Rancher cluster id no longer matches the downstream agent](../troubleshooting.md#rancher-cluster-id-no-longer-matches-the-downstream-agent).
