...
The layer-2 network this group's machines sit on, with the same keys as [`network.cluster`](network.md#networkcluster) except `kubeapi_vip`: the API VIP is a cluster-wide address — with Proxmox read from [`network.cluster`](network.md#networkclusterkubeapi_vip) or [`network.external`](network.md#networkexternalkubeapi_vip), with OpenStack the address converge reserves for the API on the tenant network — and a metal network naming its own is refused rather than silently ignored. A group on the same L2 as the VM provider's machines omits it, and a server may override it with an L2 of its own. Every metal network requires a `gateway` — the machine's links are statically addressed with DHCP off, so without one it has no default route and reaches nothing beyond its own subnet — and [`talos.kubespan`](general.md#taloskubespan) is required when the group sits on a different L2, the overlay being what carries the group's pod traffic to the rest of the cluster: the gateway there is the machine's only route to it. A group on the cluster L2 reads the gateway from [`network.cluster.gateway`](network.md#networkclustergateway), which must then be set (on a managed SDN or an OpenStack tenant network, the first host of `cidr`). A network naming the cluster L2's `cidr` describes the same wire and must agree with it on `mtu` and `vlan`, which every host on one layer-2 network shares (see [MTU](network.md#mtu)).
...
The static address of the link. Without a `/prefix` the link network's prefix length is used. On a link carrying both roles the address belongs to the `cluster` side; a dedicated `external` link's address rides its VLAN child. A `cluster` link must carry one — it is the only address the machine is known to answer on — and it must sit inside the machine's own L2, carrying that L2's prefix length when one is written, must not be that L2's network or broadcast address, and must not be one of its gateways — the machine's own route points there, and a group restating the cluster L2's `cidr` shares the wire with the VM nodes whose gateway [`network.cluster.gateway`](network.md#networkclustergateway) names. It must not be the [`kubeapi_vip`](network.md#networkclusterkubeapi_vip) or another machine's address either: the loader refuses any of these rather than letting two machines answer for one address. On a managed [SDN](proxmox.md#proxmoxnetworkclustersdn) the VM nodes' static addresses are assigned from the layout derived from [`network.cluster.cidr`](network.md#networkclustercidr) — the anycast gateway, the controlplane range and one block per worker pool — so a machine on that same L2 must take an address outside it, or a pool scaled tomorrow would claim the machine's address.
