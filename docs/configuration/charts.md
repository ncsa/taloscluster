...
`false` uninstalls the release or deletes the applied manifests, and removes the resources the plugin created for it. Namespace cleanup runs only after all entries converge successfully, so other releases keep their webhooks and Helm records during removal. A namespace another enabled entry still uses, including either Ceph driver namespace, is preserved. A disabled Gateway API entry using `version: latest` still activates cleanup when its resolved manifests are present.
