...
A node scaled up in the same run as a Kubernetes upgrade boots at the upgraded version. Existing nodes get machine configs baked at their running version so `talosctl upgrade-k8s` steps through every minor; a new node has no prior minor to step, so converge regenerates its config at the target version, otherwise it would join one or two minors behind the rest of the cluster until the next converge. Because a running cluster's configs must carry its running version — baking the target into them and applying it would skip every minor in between — converge retries a transient `kubectl version` read and refuses to generate or apply any machine configs if it still cannot establish the cluster's Kubernetes version, rather than risk a config push that skips the upgrade. The one exception is `plan` (dry-run) on a recovered management machine: it prognoses the cluster up without writing a kubeconfig, so the running version cannot be read and the plan keeps the target instead of aborting — a real run recovers the kubeconfig and steps the minors.
