...
- A VM-only cluster the harness can rewrite freely. It counts the nodes it waits for from the `controlplane` and `workers` counts alone, so a [`metal`](configuration/metal.md) section breaks it: the metal machines join the cluster as extra nodes the Ready-node assertion never expects, and `destroy` keeps the cluster's `talosconfig` for the reset the hardware needs, which fails the teardown assertion. It reads nothing but `cluster.yaml`, so the keys it edits — the upgrade phase's version bumps and the drain phase's pool count — must live there rather than in an included file, and its edits are a full YAML rewrite that drops `cluster.yaml`'s comments.
...
**Tailscale:** the scaffold adds a `tailscale:` block to both files — `cluster.yaml` sets `login_server` and `secrets.yaml` carries an `auth_key: "CHANGE-ME"` placeholder. Both must be handled before converge: (a) for the direct-access mode these examples describe, remove the `tailscale:` section from both `cluster.yaml` and `secrets.yaml` — a section in either file opts Tailscale in, and a leftover `auth_key: "CHANGE-ME"` is refused as soon as converge uses it (before any machine config is applied), while a leftover section with no auth key leaves the extension idle and converge falls back to the nodes' real addresses, which the direct-access setup routes to; or (b) supply your real headscale `login_server` and a pre-auth key. If your management machine reaches the nodes over the tailnet, keep the `tailscale: 100.64.0.0/10` entries in the security allowlists (the examples show `office:` in their place, which would lock out tailnet access).
