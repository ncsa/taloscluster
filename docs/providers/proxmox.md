...
The cluster must run **Proxmox 9 or newer**. taloscluster reads the release from the first node whenever it loads the cluster's inventory, so `converge` refuses to touch an older one and so does `destroy`, which loads the same inventory before tearing anything down: Proxmox 9 changed what an unset VM NIC MTU means, it now inherits the bridge MTU, where 8 and earlier defaulted it to 1500 and needed an `mtu=1` sentinel to inherit instead. The two conventions are each other's opposite, and the sentinel is a literal MTU of 1 on 9, so a cluster written for the wrong release loses its nodes' networking. Stay on taloscluster 0.7.x for a Proxmox 8 cluster — including to tear it down, because a newer taloscluster refuses `destroy` on it too.
