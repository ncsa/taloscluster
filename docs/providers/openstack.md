...
| Security group rule | 42 (0 with `openstack.metadata: true`) + ingress rules | the egress allowlist around the metadata service (see below) plus the `security` allowlists; the default project quota is 100 rules |
...
Converge delivers each server's machine configuration as Nova `user_data` alongside the config drive, and the configuration carries the cluster's secrets: the Kubernetes and etcd CA keys, the node join tokens, and the tailscale auth key. Nova keeps `user_data` for the instance's whole lifetime and serves it at the metadata address `http://169.254.169.254/openstack/latest/user_data`, which Neutron also answers on the IPv6 link-local address `fe80::a9fe:a9fe`. Pod egress to that address is masqueraded behind the node, so an unprivileged pod in the cluster could fetch a control plane's configuration and take over the cluster.

To close that path, the cluster security group deletes the two default allow-all egress rules Neutron seeds into every security group and instead allows egress to every destination except the metadata address — IPv4 as the 32 CIDRs covering `0.0.0.0/0` minus `169.254.169.254/32`, IPv6 as the 10 CIDRs covering `::/0` minus `fe80::/10`. Node and masqueraded pod traffic from every namespace is thereby cut off from the metadata service under any CNI, including CNIs that do not enforce NetworkPolicy, and existing clusters pick the egress rules up on their next converge. Nodes still get their configuration: the config drive carries the same `user_data`. The block relies on the cloud's Neutron backend filtering metadata traffic with port security-group rules (OVS and OVN both do), so verify it once on a fresh cluster: a node must boot, install and reboot with metadata blocked — Talos takes its hostname and network platform data from the config drive on every boot — and `curl -m5 http://169.254.169.254/openstack/latest/user_data` from a pod must time out. The egress rules add about 42 rules to the project (see the [quota table](#service-quotas)); a project that cannot spare them can set [`openstack.metadata: true`](../configuration/openstack.md#openstackmetadata) to keep the allow-all egress rules instead, leaving the metadata service, and so every node's configuration, reachable from pods.

Behind the security group, the control planes' bootstrap manifests still embed a NetworkPolicy (`block-cloud-metadata`) that denies pods in the `default` namespace egress to `169.254.169.254` — defence in depth for a CNI that enforces NetworkPolicy. A cluster bootstrapped before either existed gets the egress block on its next converge, and can add the NetworkPolicy by hand with `kubectl`. Treat membership of the project as access to the cluster's secrets regardless: Nova's default policy limits reading `OS-EXT-SRV-ATTR:user_data` through the compute API to administrators, so a project admin — or anyone the policy is widened for — can read every node's configuration.
